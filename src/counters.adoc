...
handler triggered by overflow of the lower 32 bits.
////
This is an interesting option.  This needs to be captured in the DUT Config
e.g. https://github.com/riscv-non-isa/riscv-arch-test/blob/d3546b5a430a6a9abddc1eeeae1b3a01d8c1bcdb/configs/duts/cvw/cvw-rv32imc/cvw-rv32imc.yaml#L1
////
The sample code
...
[[#norm:zicntr_rdcycleh_pseudo-op]]
RDCYCLEH is only present when XLEN=32 and reads bits 63-32
of the same cycle counter.

////
Three comments here:
1. Specifying normative rules for pseudoinstructions is dangerous as the hardware never sees them,
   and for that reason pseudoinstructions should *never* be used in a test-program. There are four
   ways to implement this pseudoinstruction: `[crsrs|csrrc|csrrsi|csrrci] rd, counter, x0` and we
   must test them all because we cannot predict how the compiler/assembler will implement a given
   pseudoinstruction.
2. It would be safer to use a specific post-fix for pseudoinstructions.  How about:
   `[#norm:zicntr_rdcycle_pseudo-op]#`
3. The sentence about RDCYCLEH is a seperate normative statement.

The above applies to all the pseudoinstructions that follow.
////
The underlying 64-bit counter should never
...
////
Is this a normative rule?  It _looks_ like one, but I am not at all certain how
to develop a test-program that can detect the number of harts per core and then
demonstrate that there is only one value of CYCLE. Also, it is my understanding
that anything captured as a Note in the ISA is not to be considered normative.
////
Precisely defining what is a "core" is
...
[[#norm:zicntr_rdtimeh_pseudo-op]]
RDTIMEH is only present when XLEN=32 and reads
bits 63-32 of the same real-time counter.

The underlying 64-bit counter
...
////
This is a normative rule, but only for multi-hart DUTs.
////
[[#norm:zicntr_multihart]]
...
example).
====
[[#norm:zicntr_rdinstret_pseudo-op]]
...
The underlying 64-bit counter should never overflow in
practice.
[NOTE]
====
...
`instret` CSR.
////
This _should_ be a normative statement!
It is absolutely worth testing for certification.
Why is it a Note?
////
====
...
////
Question: for certification, do we need to test that the high-order counters
(e.g. cycleh) increment when low-order counters (e.g. cycle) roll-over?
////

=== "Zihpm" Extension for Hardware Performance Counters

////
Is this a normative rule? How would a test-program running on the core test this?
////
