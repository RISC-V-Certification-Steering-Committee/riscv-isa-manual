...
integer division of _rs1_ by _rs2_, rounding towards zero.# [#norm:insts:rem-remu:operation]#REM and REMU
...
respectively.# [#norm:insts:remw-remuw:result_sign]#Both REMW and REMUW always sign-extend the 32-bit result
to 64 bits, including on a divide by zero.#
(((MUL, div by zero)))

The semantics for division by zero and division overflow are summarized
in <<divby0>>. [#norm:inst_group:division:div_by_zero]#The quotient of division by zero has all bits
set#, and [#norm:inst_group:remainder:div_by_zero]#the remainder of division by zero equals the dividend.# 
[#norm:inst_group:div_overflow]#Signed division overflow occurs only when the most-negative integer is divided
