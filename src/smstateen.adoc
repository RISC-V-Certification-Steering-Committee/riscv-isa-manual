...
appears as read-only zero in the matching `hstateen` and `sstateen` CSRs. [#norm:sstateen_vsmode_roz]#For every
...
[#norm:sstateen_ro1]#A bit in a supervisor-level `sstateen` CSR cannot be read-only one unless the
same bit is read-only one in the matching `mstateen` CSR and, if it exists, in
the matching `hstateen` CSR.# [#norm:hstateen_ro1]#A bit in an `hstateen` CSR cannot be read-only one
