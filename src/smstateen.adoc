...
[[norm:stateen_implicit_state_update]]
...
[#norm:stateen_csr_bit_mapping]#For every bit with a defined purpose in an `sstateen` CSR, the same bit is
...
`mstateen` CSR.# [#norm:sstateen_encroachment_ROZ]#In that case, the bit positions of "encroaching" bits will remain
...
[#norm:sstateen_read-only-1]#A bit in a supervisor-level `sstateen` CSR cannot be read-only one unless the
same bit is read-only one in the matching `mstateen` CSR and, if it exists, in
the matching `hstateen` CSR.# [#norm:hstateen_read-only-1]#A bit in an `hstateen` CSR cannot be read-only one
...
[#norm:mstateen-bit-63_read-only-zero]#Bit 63 of each `mstateen` CSR may be read-only zero only if the hypervisor
