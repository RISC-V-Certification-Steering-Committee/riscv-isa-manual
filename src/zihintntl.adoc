...
[#norm:NTL-target_definition]#The NTL instructions do not change architectural state, nor do they alter the architecturally visible effects of the target instruction.#
...
private cache in the memory hierarchy.# [#norm:NTL-P1_enc]#NTL.P1 is encoded as
ADD _x0, x0, x2_.#
...
cache in the memory hierarchy.# [#norm:NTL-PALL_enc]#NTL.PALL is encoded as ADD _x0, x0, x3_.#

[#norm:NTL-S1_op]#The NTL.S1 instruction indicates that the target instruction does not
exhibit temporal locality within the capacity of the innermost level of
shared cache in the memory hierarchy.# [#norm:NTL-S1_enc]#NTL.S1 is encoded as
ADD _x0, x0, x4_.#
...
the memory hierarchy.# [#norm:NTL-ALL_enc]#NTL.ALL is encoded as ADD _x0, x0, x5_.#
...
[#norm:C-NTL-availability]#If the C or Zca extension is provided,# [#norm:NTL-compressed_variants]# compressed variants of these HINTs are
...
[#norm:NTL-trap_behavior]#In the event that a trap is taken on the target instruction,
...
====
[norm:NTL-LR_SC_exception]#Since the NTL instructions are encoded as ADDs, they can be used within
