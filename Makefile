...
CURATED_NORM_RULES := $(BUILD_DIR)/norm-rules.json
...
#NORM_TAG_FILE_ARGS := $(foreach doc_name,$(DOCS),-t $(doc_name)$(DOC_NORM_TAG_SUFFIX))
...
NORM_RULE_CURATION_ARGS := $(foreach relative_pname,$(NORM_RULE_CURATION_FILES),-c $(relative_pname))
...
build-norm-rules: $(CURATED_NORM_RULES)
...
$(CURATED_NORM_RULES): $(DOCS_NORM_TAGS) 
	$(WORKDIR_SETUP)
